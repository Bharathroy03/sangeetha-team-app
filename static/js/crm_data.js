document.addEventListener('DOMContentLoaded', () => {
    ...
    window.filterPopupOptions = (id, query) => {
        ...
        items.forEach(item => {
            ...
            if (text.includes(cleanQuery) || role.includes(cleanQuery)) item.classList.remove('hidden');
        ...
        try {
            ...
            if (listEl) {
                listEl.innerHTML = staffList.map(u => {
                    ...
                    return `<button type="button" class="option-item flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-100 text-left group" data-value="${safe}" data-role="${u.role || ''}" onclick="selectPopupOption('crm_sales_person', '${safe}')">
            ...
            if (filterListEl) {
                ...
                filterListEl.innerHTML = defaultOption + staffList.map(u => {
                    ...
                    const initials = display.split(/[\s\-\u2014]+/).filter(w => /^[A-Za-z]/.test(w)).map(w => w[0].toUpperCase()).join('').slice(0, 2) || '??';
                    const safe = display.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
                    return `<button type="button" class="option-item flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-100 text-left group" data-value="${safe}" data-role="${u.role || ''}" onclick="selectPopupOption('crmFilterSalesPerson', '${safe}')">
                        <span class="w-7 h-7 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center flex-shrink-0 text-[10px] font-bold">${initials}</span>
                        <span class="text-xs font-semibold text-slate-700">${display}</span>
                    </button>`;
                }).join('');
